        ...
        for (uint x = idxX; x < numCases; x += gridDim.x*blockDim.x) {	
			...
			for (uint out_i = 0; out_i < sizeOut; out_i++) {
				...
				{	
					...
					float output = param*val + paramM*fmax(val, 0) + paramMN*fmax(-val, 0);
					...
					float output_1 = param_1*val + paramM_1*fmax(val, 0) +  paramMN_1*fmax(-val, 0);
...
//debug
			...
			for (uint inp_i = 0; inp_i < sizeIn; inp_i++) {	
				...
				for (uint out_i = 0; out_i < sizeOut; out_i++)	
				{
					...
					float vsign = (val > 0);
					float vsignm = (val < 0);
					float c_0 = vsign*const_area[out_par + sizeIn + inp_i]
						- vsignm*const_area[out_par + 2*sizeIn + inp_i]
						+ const_area[out_par + inp_i];
					float c_1 =
						vsign*const_area[out_par + sw_len + sizeIn + inp_i]
						- vsignm*const_area[out_par + sw_len + 2*sizeIn + inp_i]
